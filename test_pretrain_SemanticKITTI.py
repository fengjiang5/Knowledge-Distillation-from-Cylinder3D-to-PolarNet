...
def main(args):
    ...
    time_list = []
    with torch.no_grad():
        for i_iter_val,(_,_,val_grid,val_pt_labs,val_pt_fea,index, label_path) in enumerate(val_dataset_loader):
            # if i_iter_val == 5:
            #     break
    ...
    with torch.no_grad():
        for i_iter_test,(_,_,test_grid,_,test_pt_fea,test_index,label_path) in enumerate(test_dataset_loader):
            # predict
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_batch_size', type=int, default=2, help='batch size for training (default: 1)')
